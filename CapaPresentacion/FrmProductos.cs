...
namespace Proyecto
{
    public partial class FrmProductos : Form
    {
        ...
        public FrmProductos(E_Usuarios usuario)
        {
            InitializeComponent();
            _usuario = usuario;
            this.tablaProductos.DefaultCellStyle.ForeColor = Color.Black;
        ...
        public void accionesTabla()
        {
            ...
            tablaProductos.Columns[1].HeaderText = "Descripcion";
            tablaProductos.Columns[2].HeaderText = "Costo";
            tablaProductos.Columns[3].HeaderText = "Cantidad";
            tablaProductos.Columns[4].HeaderText = "Fecha_Compra";
        ...
        {
            N_Productos objNegocio = new N_Productos();
            tablaProductos.DataSource = objNegocio.ListandoProductos(buscar);
        }
        ...
        private void btnEditar_Click(object sender, EventArgs e)
        {
            ...
            {
                Editarse = true;
                txtDescripcion.Text = tablaProductos.CurrentRow.Cells[1].Value.ToString();
                txtCosto.Text = tablaProductos.CurrentRow.Cells[2].Value.ToString();
                txtCantidad.Text = tablaProductos.CurrentRow.Cells[3].Value.ToString();
                dtpFecha_Compra.Value = Convert.ToDateTime(tablaProductos.CurrentRow.Cells[4].Value.ToString());
        ...
        {
            if (AreTextBoxesFilled(txtDescripcion, txtCosto, txtCantidad))
            {
                if (Editarse == false)
                {
                    try
                    {
                        ...
                        objNegocio.InsertandoProductos(objEntidad);
                ...
                if (Editarse == true)
                {
                    try
                    {
                        objEntidad.IdProducto = Convert.ToInt32(tablaProductos.CurrentRow.Cells[0].Value);
                        objEntidad.Descripcion = txtDescripcion.Text;
                        objEntidad.Costo = Convert.ToInt32(txtCosto.Text);
                        objEntidad.Cantidad = Convert.ToInt32(txtCantidad.Text);
                        objEntidad.Fecha_Compra = dtpFecha_Compra.Value;

                        objNegocio.EditandoProductos(objEntidad);
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            ...
            {
                objEntidad.IdProducto = Convert.ToInt32(tablaProductos.CurrentRow.Cells[0].Value);
                objNegocio.EliminandoProductos(objEntidad);
...

                {
...

                    {
...

                        {
                            ...
                            PdfPTable pTable = new PdfPTable(tablaProductos.Columns.Count);
                            ...
                            foreach (DataGridViewRow viewRow in tablaProductos.Rows)
        ...
        private void btnExcel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    DataTable dt = Excel.DataGridView_To_Datatable(tablaProductos);
